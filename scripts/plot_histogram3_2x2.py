...
# ============================================================
# FIGURE 1: TRANSFORMED HISTOGRAMS (SQUARE GRID)
# ============================================================
k = len(selected_N)
fig, axes, nrows, ncols = make_grid_axes(k, base=4.0, sharey=True)
...
for i, N in enumerate(selected_N):
    ...
    ax.plot(x, y, linestyle="dashed", color="red", alpha=0.8, label="Std Gumbel")

label_left_column(axes, ncols, "Density")
hide_unused_axes(axes, k, nrows, ncols)
plt.tight_layout()
...
for i, N in enumerate(selected_N):
    ...
    for r in r_values:
        ...
        X0_bis = int(np.floor(N - N ** 0.25))

        t, h, g, f = fft_convolution_density(r, X0, X0_bis, N, tmax=100, dt=0.01)
        z2t = (N - X0) * (1 / r - 1) ** 2 * np.exp(-(1 / r - 1) * t) * (
            (1 - np.exp(-(1 / r - 1) * t)) ** (N - X0 - 1) /
            (1 / r - np.exp(-(1 / r - 1) * t)) ** (N - X0 + 1)
        )

        p0_approx = 1 - r ** (N - X0)

        ax.plot(t, z2t + p0_approx * h, linestyle="dashed", color="b", alpha=0.8, label="density mixture")
