...
for i, N in enumerate(selected_N):
    ...
    for r in r_values:
        #times = subsetN.loc[np.isclose(subsetN["r"], r) & (subsetN["AbsorbingState"] == N) ,"AbsorptionTime"].to_numpy()
...
def f_gumbel_shift_scale(t, r, X0_bis, N):
    # uses your c formula; keeps your structure but fixes undefined r
    c = -r * np.log(1.0 - X0_bis / N) + np.log(X0_bis * (1.0 - r)) # -1.5
    # NOTE: check signs here against your intended Gumbel form
...
for i, N in enumerate(selected_N):
    ...
    for r in r_values:
        #times = subsetN.loc[    np.isclose(subsetN["r"], r) & (subsetN["AbsorbingState"] == N),    "AbsorptionTime"].to_numpy()
        #times = subsetN.loc[    np.isclose(subsetN["r"], r) & (subsetN["AbsorbingState"] == 0),    "AbsorptionTime"].to_numpy()
        times = subsetN.loc[np.isclose(subsetN["r"], r), "AbsorptionTime"].to_numpy()
        if times.size == 0:
            continue
        ...
        #axes2[i].plot(x2, z2/pN_approx, linestyle="dashed", color="blue", alpha=0.8, label="T density")
        #axes2[i].plot(x2, y2, linestyle="dashed", color="b", alpha=0.8, label="T density")

        X0_bis = int(np.floor(N - N**.25))

        t, h, g, f = fft_convolution_density(r, X0, X0_bis, N, tmax=100, dt=0.01)
        z2t = (N-X0) * (1/r-1)**2 * np.exp(-(1/r-1)*t) * ( (1-np.exp(-(1/r-1)*t))**(N-X0-1) / (1/r - np.exp(-(1/r-1)*t))**(N-X0+1))

        #axes2[i].plot(t, z2t + p0_approx*h, linestyle="dashed", color="b", alpha=0.8,label="convolution h")
        axes2[i].plot(t, z2t + p0_approx*h, linestyle="dashed", color="b", alpha=0.8,label="density mixture")


        axes2[i].hist(  times,      bins=50,   density=True,     alpha=0.5,  histtype="stepfilled",  edgecolor="black",            label=f"r={r:.2f}",
        )
